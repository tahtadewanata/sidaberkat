      ...
      <div class="card-header pb-0 p-3">
        <div class="row">
          <div class="col-6 d-flex align-items-center">
            <h6 class="mb-0">Master Pokok Masalah</h6>
          </div>
          ...
          <table class="table align-items-center mb-0">
            <thead>
              <tr>
                ...
                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nama Pokok Masalah</th>
            ...
            <tbody>
             <?php
             $count=0; 
             foreach ($get_master_pokok_masalah as $val) { 
              $count=$count+1;
              ?>
              <tr>
                <td style="text-align:center;"><?= $count; ?></td>
                <td class="align-middle text-center text-sm"><span class="badge badge-sm bg-gradient-info"><?= $val->nama_pokok_masalah; ?></span></td>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Form Pokok Masalah</h1>
      ...
      <div class="modal-body">
        <form id="form_input">
          <div class="row">
            <div class="col-lg-12">
              <div class="mb-3">
                <label for="teammembersName" class="form-label">Nama Pokok Masalah</label>
                <input type="hidden" class="form-control" id="indikator" name="indikator">
                <input type="hidden" class="form-control" id="id_edit" name="id_edit">
                <input type="text" class="form-control" id="nama_pokok_masalah" name="nama_pokok_masalah">
              </div>
            </div>
            <div class="col-lg-12">
...
<script>
  function save_data(){
    ...
    var nama_pokok_masalah = $("#addmembers").find("input[name='nama_pokok_masalah']").val();

    var url='';

    if(indikator==69){
      url='<?= base_url();?>MasterPokokCon/edit_artikel/'+id;
    }
    else{
      url='<?= base_url();?>MasterPokokCon/save_data';
    }


    if(nama_pokok_masalah == "")
    {
      ...
      form_data.append('nama_pokok_masalah', nama_pokok_masalah);
  ...
  function update(id){
    ...
    $.ajax({
      dataType: 'json',
      url: '<?= base_url();?>MasterPokokCon/getById/'+id,
      success: function ( data) {
        console.log(data);
        $("#addmembers").find("input[name='indikator']").val(69);
        $("#addmembers").find("input[name='id_edit']").val(data.data[0].id);
        $("#addmembers").find("input[name='nama_pokok_masalah']").val(data.data[0].nama_pokok_masalah);
  ...
  function hapus(id){
    $.ajax({
      ...
      url: '<?= base_url();?>MasterPokokCon/getById/'+id,
      success: function ( data) {
        var nama_pokok_masalah = data.data[0].nama_pokok_masalah;
        console.log(nama_pokok_masalah);
        $.confirm({
          title : 'Hapus Data',
          content : '<strong>Anda Yakin Untuk Menghapus Data :</strong> '+nama_pokok_masalah,
          buttons: {
            hapus: {
              btnClass: 'btn-blue',
              text:'Hapus',
              action: function(){
                $.ajax({
                  dataType: 'json',
                  type:'POST',
                  url: '<?= base_url();?>MasterPokokCon/delete/'+id,
                  data:{
                    id:id
                  }
                }).done(function(data){
